...
def is_diffs_within_normal_range(diff_ppl, threshold=5.0):
    return not any(diff < (-threshold) for diff in diff_ppl if isinstance(diff, float))
...
def main():
    ...
    latest_csv[f'FP16'] = latest_csv['Model'].apply(lambda model: fp16_dict.get(model, {}).get('ppl', 'N/A'))
    latest_csv[f'diff_FP16(%)'] = latest_csv.apply(lambda row: calculate_percentage_difference(row['ppl'], row[f'FP16']), axis=1)
    ...
    if len(csv_files)>1:
        ...
        last_ppl=['']*len(latest_csv.index)
        diff_ppl=['']*len(latest_csv.index)
        ...
        for csv_file in csv_files:
            ...
            for current_csv_ind,current_csv_row in current_csv.iterrows():
                ...
                current_csv_model_ppl=current_csv_model+'-'+current_csv_precision+'-'+'ppl'
                add_to_dict(csv_dict, current_csv_model_ppl, current_csv_row['ppl'])
        ...
        for latest_csv_ind,latest_csv_row in latest_csv.iterrows():
            ...
            for previous_csv_ind,previous_csv_row in previous_csv.iterrows():
                ...
                if latest_csv_model==previous_csv_model and latest_csv_precision==previous_csv_precision:
                    ...
                    previous_ppl=previous_csv_row['ppl']
                    if previous_ppl > 0.0:
                        last_ppl[latest_csv_ind]=previous_ppl
                        diff_ppl[latest_csv_ind]=round((latest_ppl-previous_ppl)*100/previous_ppl,2)
                        in_previous_flag=True

            if not in_previous_flag:
                last_ppl[latest_csv_ind]=pd.NA
                diff_ppl[latest_csv_ind]=pd.NA

        latest_csv.insert(loc=9,column='last_ppl',value=last_ppl)
        latest_csv.insert(loc=10,column='diff_ppl(%)',value=diff_ppl)


        diffs_within_normal_range = is_diffs_within_normal_range(diff_ppl, threshold=highlight_threshold)

        subset1=['diff_ppl(%)']
        
        columns={'ppl': '{:.2f}', 'last_ppl': '{:.2f}', 'diff_ppl(%)': '{:.2f}'}
        ...
        styled_df = styled_df.applymap(lambda val: highlight_vals(val, max=highlight_threshold, is_last=False), subset=[f'diff_FP16(%)'])
